...
def build(input_path, output_path):
    ...
    stores = sorted(store_map.values(), key=lambda x: (x['BRAND'], x['STORE_NAME']))
    n_stores = len(stores)
...
def main():
    ...
    parser.add_argument('--input',  required=True,  help='Path to store_services_enriched.csv')
    parser.add_argument('--output', required=True,  help='Output .xlsx path (include date in name)')
