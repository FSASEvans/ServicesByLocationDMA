...
recent_transactions AS (
    ...
    FROM CINCH_DATA.CINCH_TO_FULLSPEED_AUTOMOTIVE_DATA_SHARE.TRANSACTIONS AS t
    LEFT JOIN CINCH_DATA.CINCH_TO_FULLSPEED_AUTOMOTIVE_DATA_SHARE.CUSTOMERS AS c
        ON t.CUSTOMER_REF_ID = c.CUSTOMER_REF_ID
    WHERE t.TRANSACTION_AT >= DATEADD('month', -3, CURRENT_DATE())
      AND (c.ENTITY_GROUP IS NULL OR TRIM(c.ENTITY_GROUP) = '')
),
